...
| **Service: Catalogue Service & Tokens** | **VERIFIED** | `scripts/audit-student-settings-access.js` | None. Token granted to any authenticated user. |
...
| **Auth: 3-Level RBAC (ANON/STUDENT/ADMIN)** | **VERIFIED** | `audit-auth-flow.js`, `audit-security-auth.js`, `audit-console-owner-only.js` | None. Server-side only; forged claims rejected. |
| **Student Settings: Catalogue Access** | **VERIFIED** | `audit-student-settings-access.js` | None. Any authenticated Google user = catalogue editor. |
| **Roll Persistence: Server-side Authoritative** | **VERIFIED** | `audit-roll-persistence.js` | None. Anonymous roll never persisted; sign-out wipes. |
| **Activity Auditing: Full Event Lifecycle** | **VERIFIED** | `audit-activity-logging.js` | None. OTP strictly excluded; 7 categories supported. |
| **Console: Owner-Only Access Enforcement** | **VERIFIED** | `audit-console-owner-only.js` | None. 8 admin endpoints; forged headers rejected. |
...
- **Security Audit Scripts**: 6 (`audit-auth-flow.js`, `audit-security-auth.js`, `audit-student-settings-access.js`, `audit-roll-persistence.js`, `audit-activity-logging.js`, `audit-console-owner-only.js`) — all PASS
