/* =============================================================================
   experiment-cover.js — Chittagong University Experiment Cover Page Generator
...
(function (global) {
  ...
  var elementIds = [
    ...
    'studentSubVal', 'coverPage', 'previewScale', 'emptyHint', 'previewContainer',
    'bridgeDot', 'bridgeText', 'themeToggleBtn', 'historyBtn', 'historyCount',
    'historyDrawer', 'historyDrawerBackdrop', 'historyDrawerClose', 'historyList',
    'tabEditor', 'tabPreview', 'mobileTabs', 'decCopies', 'incCopies',
    'colorModeToggle', 'costCalcText', 'costTotalTag', 'chipSubToday',
  ...
  function populateCourseDropdown() {
    ...
    if (!labList.length) {
      ...
      if (el.experimentSelect) {
        el.experimentSelect.innerHTML = '<option value="">No experiments</option>';
        el.experimentSelect.disabled = true;
      }
      updatePreview();
      return;
    }
  ...
  function selectCourse(code) {
    ...
    populateExperimentDropdown(c);
    updatePreview();
  }

  function populateExperimentDropdown(courseObj) {
    if (!el.experimentSelect) return;
    el.experimentSelect.innerHTML = '';
    el.experimentSelect.disabled = false;

    var exps = (courseObj && courseObj.experiments) ? courseObj.experiments : [];
    if (!exps.length) {
      var opt = document.createElement('option');
      opt.value = '';
      opt.textContent = 'No experiments listed for this course';
      el.experimentSelect.appendChild(opt);
      current.experiment = null;
      return;
    }
    ...
    exps.forEach(function (exp, idx) {
      ...
      opt.textContent = 'Exp ' + num + ': ' + title;
      el.experimentSelect.appendChild(opt);
    });

    var resolvedExp = (global.LabDDB && global.LabDDB.catalogue)
      ? global.LabDDB.catalogue.resolveExperimentSelection(exps, userSelectedExpIndex)
      : { index: exps.length - 1, experiment: exps[exps.length - 1] };

    if (resolvedExp && resolvedExp.experiment) {
      el.experimentSelect.value = String(resolvedExp.index);
      current.experiment = resolvedExp.experiment;
    } else {
      el.experimentSelect.value = '0';
      current.experiment = exps[0];
    }
    updatePreview();
  }

  function selectExperiment(idx) {
    if (!current.course || !current.course.experiments) return;
    var exp = current.course.experiments[parseInt(idx, 10)];
    if (exp) {
  ...
  function updatePreview() {
    ...
    // Experiment info
    if (exp) {
      setField('experiment-no', esc(exp.num || '01'));
      setField('experiment-name', esc(exp.title || exp.expTitle || '---'));
    } else {
      setField('experiment-no', '---');
      setField('experiment-name', '---');
    }
    ...
    // Dates
    var expD = el.experimentDate && el.experimentDate.value ? formatDate(el.experimentDate.value) : '';
    var subD = el.submissionDate && el.submissionDate.value ? formatDate(el.submissionDate.value) : '';
    setField('experiment-date', expD);
  ...
  function getCoverFilename() {
    ...
    var expNum = (current.experiment && current.experiment.num) ? current.experiment.num : (el.experimentSelect ? el.experimentSelect.value : '01');
    ...
    if (global.Uprint && global.Uprint.formatCoverFilename) {
      return global.Uprint.formatCoverFilename(dept, code, expNum, roll);
    }
    ...
    Uprint.elementToPdfBase64(el.coverPage).then(function (b64) {
      ...
      if (global.LabDDB && global.LabDDB.auth && global.LabDDB.auth.logActivity) {
        global.LabDDB.auth.logActivity('PDF_DOWNLOADED', { type: 'cover', id: (current.course && current.course.courseCode) || 'CU' }, {
          ...
          roll: (el.rollNumber && el.rollNumber.value) ? el.rollNumber.value.trim() : '',
          tool: 'experiment-cover',
        });
      }
  ...
  function handleOtp() {
    ...
    Uprint.requestPrint({
      ...
      tool: 'experiment-cover',
      title: current.experiment ? current.experiment.title || 'Lab Report' : 'Lab Report',
      courseCode: code,
      roll: roll,
  ...
  function initEvents() {
    if (el.courseSelect) {
      el.courseSelect.addEventListener('change', function () {
        ...
        userSelectedExpIndex = null;
        selectCourse(this.value);
      });
    }

    if (el.experimentSelect) {
      el.experimentSelect.addEventListener('change', function () {
        userSelectedExpIndex = this.value;
        selectExperiment(this.value);
      });
    }
    ...
    if (el.experimentDate) {
      el.experimentDate.addEventListener('change', updatePreview);
    }
    if (el.submissionDate) {
